...
```
All 4 models
A1 - Linearity: The linear model is giving residuals that are approximately normally distributed. 
...
```{r vif test}
lm.model <- lm(total_fatality_rate ~ year + bac08 + bac10 + 
                      adm_lic_revoc_law + primary_seatbelt_law + 
                      secondary_seatbelt_law + sl75 + slnone + 
                      grad_drivers_lic_law + percent_pop_aged_14_to_24 + 
                      log(unemployment_rate) + log(miles_driven_per_capita), 
...
The independent variables have non-zero variance and not too many extreme values.
...
We don’t have evidence to reject the null hypothesis of no serial correlation in the between and first difference models. We do have evidence to reject the null hypothesis of no serial correlation in the within and pooled OLS models. 
