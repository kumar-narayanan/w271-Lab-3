...
All the models are consistent in the negative sign of the Blood Alcohol variable coefficient. The Pooled OLS and Within models significance levels are higher compared to the First Difference and Between models. The standard errors are lowest for the Within model and highest for the Between model.
...
residuals vs fitted
corr matrix
From the below Q-Qplot the residuals are approximately normally distributed for all the 4 models.
...
```
Proof: The regressor have non-zero variance and not too many extreme values (but constant)
Based on the below variance information, the independent variables have non-zero 
variance and not too many extreme values.
...
We don’t have evidence to reject the null hypothesis of no serial correlation in the between models. We do have evidence to reject the null hypothesis of no serial correlation in the within and pooled OLS models. 
For first difference estimator model, Durbin-Watson and Breusch-Godfrey/Wooldridge test shows contradictory results.


```{r check for exogenity}
df <- traffic_df
...
for (t in unique(df$year)){
  for (var_name in colnames(pool.model$model)[c(-1)]){
    for (year_s in unique(df$year)){
      c = cov(df[df$year == t,][var_name], df[df$year == year_s,]$resid_model)
...
Based on the pFtest result, p-value of 0.002243, we reject the null hypothesis of no fixed effects. 
...
```{r}
colin_data = traffic.new.row 
colin_data$logmiles_driven = log(colin_data$miles_driven_per_capita)
colin_data$logunemp = log(colin_data$unemployment_rate)

colin_data = colin_data %>% select(year,adm_lic_revoc_law, primary_seatbelt_law,secondary_seatbelt_law,sl75,slnone,grad_drivers_lic_law, percent_pop_aged_14_to_24,logunemp, logmiles_driven)
corrplot(cor(colin_data), method="color", number.cex=0.5, tl.cex=0.5, type='lower')
```
...
**The test reveals serial correlation - the p-value does not reject the null hypothesis of no serial correlation. Thus the computed standard errors is likely to not have serial correlation.**

**Plotting the resideuals below we do not see a clear trend of divergence in errors across the index, helping support the above test that there is a lack of serial correlation in the model** 

```{r}
plot(between.model$residuals)
```
